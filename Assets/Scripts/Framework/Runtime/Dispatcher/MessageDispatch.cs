...
public class MessageDispatch
{
    ...
    {
        if (obj is Transform transObj)
        {
            var tmp = transObj.GetComponent<IMsgObj>();
            if (tmp == null) return;
        }
        else if (obj is GameObject goObj)
        {
            var tmp = goObj.GetComponent<IMsgObj>();
            if (tmp == null) return;
        }
        else if (obj is not IMsgObj)
            return;

        var methods = obj.GetType().GetMethods(BindingFlags.Instance |
            BindingFlags.NonPublic |
            BindingFlags.Public |
            BindingFlags.Static).Where(item => Attribute.IsDefined(item, typeof(CmdCallbackAttribute)));
        ...
        foreach (var method in methods)
        {
            ...
            {
                if (attribute is CmdCallbackAttribute messageAttribute)
                {
...
public class CmdCallbackAttribute : Attribute
{
    public ushort MessageId { get; private set; }
    public int Channel { get; private set; }
    public CmdCallbackAttribute(ushort messageId, int channel = -1)
    {
        MessageId = messageId;
        Channel = channel;
    }
