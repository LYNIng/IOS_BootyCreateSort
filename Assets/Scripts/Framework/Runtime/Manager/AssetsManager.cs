...
public class AssetLoader<T> : ILoadAsset where T : UnityEngine.Object
{
    ...
    public void OnLoaded<TObject>(TObject obj)
    {
        ...
        {
            Debug.LogError($"Type mismatch: Expected {typeof(T)}, got {obj?.GetType()}");
        }
    }
    public AssetLoader(object assetPath)
    {
        ...
        if (assetPath.ToString().Length == 0)
...
{
    private AsyncOperationHandle<IList<Sprite>> _LoadSpriteHandle;
    ...
    public void Unload()
    {
        if (Handle.IsValid())
        {
            Addressables.Release(Handle); // 使用正确的释放方法
            Handle = default;
        }
        if (_LoadSpriteHandle.IsValid())
        {
            Addressables.Release(_LoadSpriteHandle);
            _LoadSpriteHandle = default;
        }
    ...
    public async Task AsyncLoad()
    {
        ...
        try
        {
            ...
            if (_needLoadSprite)
            {
                _LoadSpriteHandle = Addressables.LoadAssetsAsync<Sprite>(
                    AssetPaths.AsEnumerable(),
                    OnLoaded,
    ...
    private bool _needCheckLoadSprite = true;
    private bool _needLoadSprite = false;
    public void OnLoaded<TObject>(TObject obj)
    {
        //Debug.Log($"---> 加载 {obj} ");
        if (obj is T result)
        {
            //在移动端平台上 同名的Sprite 和 Texture2D 会冲突 所以这里做个处理
            if (_needCheckLoadSprite)
            {
                if (result is Texture2D)
                {
                    _needLoadSprite = true;
                }
                else if (result is Sprite)
                {
                    _needLoadSprite = false;
                    _needCheckLoadSprite = false;
                }
            }
    ...
    public bool TryGetPreloadAsset<AssetType>(string assetName, out AssetType resultAsset) where AssetType : UnityEngine.Object
    {
        if (Assets.TryGetValue(assetName, out var result))
        {
            ...
            else
            {
                ...
                Debug.Log($"TryGetPreloadAsset {assetName} 类型错误 调用是{typeof(AssetType)} - 资源是{result.GetType()}");
            }
        }
        resultAsset = null;
        return false;
...
public class AssetsManager : MonoSingleton<AssetsManager>, IManager
{
    ...
    public static async void LoadAsset<T>(string path, Action<T> onLoaded)
    {
        var result = await AsyncLoadAsset<T>(path);
    ...
    public static async Task<IPreloadAssetLoader> AsyncCreatePreloadAssetsLoader(Type preloadType)
    {
        ...
        if (objType.TryGetCustomAttribute(out PreloadAssetsAttribute result))
        {
            ...
            if (loaderPro != null && loaderPro.PropertyType == typeof(IPreloadAssetLoader) && paths != null && paths.Count > 0)
            {
...
public class PreloadAssetsAttribute : Attribute
{
    ...
    {
        this.loadSwitch = AppExcuteFlagSettings.ToBFlag;
