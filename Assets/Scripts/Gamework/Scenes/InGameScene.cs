...
public class GamePlay : Singleton<GamePlay>, IMsgObj
{
    ...
    {
        InGameScene.Instance.HomePage.SetActive(true);
        InGameScene.Instance.GamePage.SetActive(false);
    }

    public void ShowGamePage()
    {
        InGameScene.Instance.HomePage.SetActive(false);
        InGameScene.Instance.GamePage.SetActive(true);
    }
    ...
    private async Task OnGamePlay_Begin()
    {
        ...
        var loadingUI = await UIManager.OpenUIAsync<User_Loading>();

        ShowGamePage();
        await UIManager.CloseUIAsync<User_Home>();

        await GamePlay_ShelfGame.Instance.InitData(GlobalSingleton.Level);
        await UIManager.OpenUIAsync<User_GamePlay>();
        ...
        if (GlobalSingleton.GuideState == 1)
        {
            ...
            {
                guideIDX = 1,
                goods = GamePlay_Storage.Instance.guideTempGoodsList[0]
            });
            ...
            {
                guideIDX = 1,
                goods = GamePlay_Storage.Instance.guideTempGoodsList[1]
            });
            ...
            {
                guideIDX = 1,
                goods = GamePlay_Storage.Instance.guideTempGoodsList[2]
            });
            await guideUI.WaitInteraction();
    ...
    private async Task OnGamePlay_LevelComplete()
    {
        ...
        var ui = await UIManager.OpenUIAsync<User_Loading>();
